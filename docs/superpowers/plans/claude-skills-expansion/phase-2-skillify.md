# Phase 2 — Skillify Skill

> **Part of:** [Claude Skills Expansion Master Plan](../2026-04-06-claude-skills-expansion.md)

**Goal:** Create `/skillify` — a skill that analyzes a session and converts the workflow into a reusable SKILL.md file.

**Files:**
- Create: `claude/skills/skillify/SKILL.md`

**Depends on:** Phase 1 (skill-creator plugin installed)

---

### Task 5: Create and deploy the Skillify skill

- [ ] **Step 1: Create `claude/skills/skillify/SKILL.md`**
...
- [ ] **Step 2: Deploy via install.sh**

```bash
bash install.sh 2>&1 | grep -i skill
```

Expected output includes: `→ Copying skills...`

- [ ] **Step 3: Verify deployment and verify.sh picks it up**

```bash
ls ~/.claude/skills/skillify/SKILL.md && bash verify.sh 2>&1 | grep "skillify"
```

Expected:
```
/c/Users/<user>/.claude/skills/skillify/SKILL.md
  ✅ skillify skill
```
...
```bash
git add claude/skills/skillify/SKILL.md
git commit -m "feat: add skillify skill — converts sessions into reusable skills"
git push
```
