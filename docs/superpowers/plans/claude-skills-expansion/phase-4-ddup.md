# Phase 4 — DDUP Command

> **Part of:** [Claude Skills Expansion Master Plan](../2026-04-06-claude-skills-expansion.md)

**Goal:** Create `/ddup <issue-number>` — a command that detects duplicate GitHub issues via Claude-based semantic comparison and drafts a comment, requiring human confirmation before posting.

**Files:**
- Create: `claude/commands/ddup.md`

**Depends on:** Phase 1 (`gh` CLI already installed; verified in verify.sh)

---

### Task 7: Create and deploy the DDUP command

- [ ] **Step 1: Create `claude/commands/ddup.md`**
...
- [ ] **Step 2: Deploy and verify the file exists**

```bash
bash install.sh && ls ~/.claude/commands/ddup.md
```
...
```bash
git add claude/commands/ddup.md
git commit -m "feat: add ddup command — detect duplicate GitHub issues via semantic analysis"
git push
```
