...
| Gate | Holder | Status |
|---|---|---|
| **host-live / HA-live / exposure** | **`AN-01` G1** (branch `homebrain/an-01-implementation-plan`) | **CLAIMED 2026-09-07** — G1 discovery. **G1b done** (D8 settled — `intent`/`params` are top-level `data:` keys, no `payload:` wrapper; only `rest_command.resolver_command` exists; **D13 still open**, needs a YAML read). **SPIKE-AN-3 in progress — a THROWAWAY automation is LIVE in HA:** `automation.an_01_spike_an_3_source_discriminator_throwaway_delete_after` (id `an01spikean3`), triggered by the phrase *“zibble wombat probe”*, which does nothing but `system_log.write` to logger `an01.spike`. It is waiting on the operator to speak that phrase from the phone, the satellite and web Assist. **To remove it:** `DELETE /api/config/automation/config/an01spikean3`, or the HA UI. AN-2 and AN-1 deferred to an attended session. AN-2 and AN-1 deferred to an attended session. Release when AN-3’s automation is deleted and its result is recorded at Checkpoint A. Prior: **FREE** — `S1b-1′` claimed it to deploy the resolver `say` rework and released it on completion (2026-07-16; **Spike-2 not passed** — `play_announcement` silent on the ceiling, see `CHANGELOG.md` 2026-07-16). Prior: `S1a` (2026-07-15). |
