...
| Gate | Holder | Status |
|---|---|---|
| **host-live / HA-live / exposure** | **`AN-01` G1 — AN-2/AN-1** (branch `homebrain/an-01-implementation-plan`) | **CLAIMED 2026-09-07** — attended AN-2 session in progress: satellite mic-mute switch (`switch.respeaker_living_room_microphone_mute`) is being toggled and the reSpeaker may be power-cycled for the D14 test. AN-1 (audible chime) may follow under separate approval. **If interrupted:** verify that switch is back `off` and the reSpeaker is powered on. Prior: **FREE** — released 2026-09-07 by `AN-01` G1. **G1b done:** D8 settled (`intent`/`params` are top-level `data:` keys, `params` structured, no `payload:` wrapper); only `rest_command.resolver_command` exists; **D13a/D13b still OPEN** — the `timeout:` and the `payload:` body template need a YAML read (no config entry, no API, no VM shell), and D13b is a stop gate that blocks Checkpoint A. **SPIKE-AN-3 done — D12 PASSES.** Three sources, one probe sentence: phone `device_id=1542a2a3…` (SM-S948W-Costea) / `satellite_id=None`; satellite `device_id=b30ac5e3…` (reSpeaker Living Room) / `satellite_id=assist_satellite.respeaker_living_room_assist_satellite`; web Assist `device_id=None` / `satellite_id=None`. `device_id` separates all three and was stable over three phone repeats; `satellite_id` separates satellite from not. `agent_id`, `user_id` and `details` carry nothing. Throwaway automation `an01spikean3` **deleted** (verified 0 remaining). **AN-2 and AN-1 NOT run** — D1–D6 and D14 still open, deferred to an attended session. Prior: **FREE** — `S1b-1′` claimed it to deploy the resolver `say` rework and released it on completion (2026-07-16; **Spike-2 not passed** — `play_announcement` silent on the ceiling, see `CHANGELOG.md` 2026-07-16). Prior: `S1a` (2026-07-15). |
