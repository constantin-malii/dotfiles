...
| Gate | Holder | Status |
|---|---|---|
| **host-live / HA-live / exposure** | *(none)* | **FREE** — released 2026-09-07. **G1 COMPLETE: AN-3, G1b, AN-2 and AN-1 all done; Checkpoint A closed; G2 unblocked.** **AN-1 PASSES** — the chime blocker standing since 2026-09-05 was a **`./` in the media_content_id**: HA signs the un-normalised path but returns a normalised URL, so the signature cannot validate (proven both ways). With `media-source://media_source/local/timer_chime.wav` the play returned 200, reached `playing` in 0.16s, and the **operator heard the bell on the ceiling**. D5: MA **preserves** the query, echoing `builtin://track/<url>?authSig=…`, so design §8.2’s path-only match key is unnecessary. Two hazards recorded in the plan: the echoed cid **contains a bearer credential** (needs explicit log redaction, not truncation), and `_is_reply_uri` does not recognise a `builtin://track/` chime so `resume` could replay a spent one. `CHANGELOG.md:246-252`’s “MA cannot fetch HA media files” conclusion was downstream of the `./` and gets a dated correction at G5. Ceiling volume raised to 60% for the audibility test and **restored to 10%** (verified); radio replayed. Prior: claimed for SPIKE-AN-1.
