...
| Gate | Holder | Status |
|---|---|---|
| **host-live / HA-live / exposure** | *(none)* | **FREE** — released 2026-09-07 after the attended **AN-2** session. **AN-2 COMPLETE, all four go criteria passed:** D1 `switch.respeaker_living_room_microphone_mute`; **D2 PASS** (zero pipeline runs on either slot while muted, with a positive control once unmuted, and the LED ring turns red while muted); **D6** no audible feedback observed (measured with speakers connected at 50%, so `mute_unmute_sound` can stay on); **D14 PASS** (HA marked the device unavailable 45s after a power unplug and never faked `on` — `switch.turn_on` returned HTTP 200 with the entity staying `unavailable` across 18s, so the design 9.3 read-back is meaningful). Mic-mute confirm latency **255ms**, so the shipped 2000ms budget stands. Satellite left powered, mute `off`, no config changed. **AN-1 NOT run** — D3/D4/D5 open; it needs a quiet house because its go criterion is hearing the chime. Checkpoint A blocks only on those three. Prior: **FREE** — released 2026-09-07 by `AN-01` G1 after G1b + SPIKE-AN-3.
