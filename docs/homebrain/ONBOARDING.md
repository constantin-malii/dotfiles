...
3. **File the upstream issue (highest value now)** against `music-assistant/server` with the §7 evidence: *MA holds the player playback lock during the entire (slow) stream resolution → concurrent/overlapping commands time out 30 s ("previous holder appears stuck"); also the cold-start latency.* **Suggested fix:** resolve the stream **outside** the playback lock; hold the lock only to start/stop the player. (Confirm with user before submitting.)
