...
| 2026-06-24 | **Stop-path hypothesis REFUTED by live trace.** Set squeezelite `log_level=VERBOSE` and traced **6/6 clean radio play→stop cycles** — every stop returned both players to `idle` via **`STMf` (connection closed)**, not `STMu`. So the `power(False)`-no-op / "only STMu→STOPPED" hypotheses are wrong; the normal stop path works. The wedge is **intermittent / condition-dependent** (contended/interrupted/errored stops — rapid plays, stop during buffering before `STMs`, source switch, or a 500'd/dropped stream), **not** the clean stop. **No upstream bug report filed.** log_level reverted to GLOBAL. Updated `research-playback-lock.md` + `ONBOARDING.md`. |
| 2026-06-24 | **Interrupted-state experiment → ROOT CAUSE FOUND.** 6 interrupt-during-resolution conditions (stop/2nd-play/radio/announce/clear during YTM setup + radio→YTM→stop): **all 6 reproduced "previous holder appears stuck" lock timeouts** (worst c4=11); **no persistent Universal/protocol state mismatch** (states converge to idle). Real reproducible defect = **the slow YTM stream-resolution holds the player playback lock for its full ~14–150 s duration**, so any overlapping command blocks 30 s. Lock is held-during-resolution (not dead). This supersedes the refuted `power(False)` theory and also explains the cold-start latency. Suggested upstream fix: resolve the stream *outside* the lock. Detail in `research-playback-lock.md` §7. |
| 2026-06-24 | **Upstream research + issue draft.** Confirmed in MA source: `@handle_play_action` wraps `play_index`→`_load_item`→`get_stream_details` (slow YTM resolution) **inside** the PLAYBACK lock; the 30 s timeout/message came from PR #4024. **No matching upstream issue exists** (novel/fileable). Workarounds: pause avoids the lock (not stop); pre-resolve/warm-cache NOT feasible; local patch feasible but deferred to upstream. Prepared `upstream-issue-draft.md` (NOT submitted — awaiting review). Detail in `research-playback-lock.md` §8. |
