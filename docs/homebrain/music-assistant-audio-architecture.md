...
**Open issue — YouTube Music cookie rotation:** YTM auth via login cookie is unstable; YouTube rotates/invalidates the cookie (MA log: *"The provided YouTube account cookies are no longer valid … rotated … as a security measure"* → *"User does not have Youtube Music Premium"*), which makes the YTM provider fail to load and return no results. Workaround: extract the cookie from a private/incognito window and close it without logging out (reduces rotation). Until YTM auth is stable, the assistant stays radio-only and YTM is **not** exposed to the LLM. Local library + radio are unaffected.
